...
fn simulate(reg_bank: &mut RegBank, memory: &mut Vec<u8>, pc: &mut usize) {
    loop {
        ...
        *pc += 2;
        ...
        match curr_instruction {
            ...
            Instruction::Ceq(reg1, reg2) => {
                ...
                reg_bank.set_equal_flag(comparator_result);
            }
            Instruction::Clt(reg1, reg2) => {
                let comparator_result: bool = reg_bank[reg1] < reg_bank[reg2];
                reg_bank.set_less_flag(comparator_result);
            }
            ...
            Instruction::Beq(reg1) => {
                if reg_bank.get_equal_flag() {
                    *pc = reg_bank[reg1] as usize;
                }
            }
            Instruction::Bne(reg1) => {
                if reg_bank.get_less_flag() {
                    *pc = reg_bank[reg1] as usize;
                }
            }
...
fn main() -> std::io::Result<()> {
    ...
    let mut pc = file_header.entry_point as usize;

    // Executing instructions
    simulate(&mut reg_bank, &mut memory, &mut pc);
    println!();
